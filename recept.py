...
def complex_delta(cval, prior_cval):
    return cval / prior_cval if prior_cval != 0.0 else 0.0
...
class Distribution:
    ...
    def __init__(self, initial_value=0.0, prior_sequence=0.0):
        self.ave = ExponentialSmoother(initial_value)  # mean average
        self.dev = ExponentialSmoother(prior_sequence)  # standard deviation
...
class TimeApex:
    """
    Returns the given sample, and time since last given sample, if it is changing direction.
    That is, return on the derivative changing sign.
    """
...
class DynamicWindow:
    ...
    def __str__(self):
        return "{window=%08.3f, duration=%s, target=%r}" % (
            (self.td / self.ed.v.v) if self.ed.v.v > 0.0 else 0.0, self.ed, self.td)
...
class TimeSmoothing:
    ...
    def __init__(self, period, phase, window_factor=1.0, initial_value=0.0 + 0.0j):
        self.period = period
        self.phase = phase
        self.v = ExponentialSmoother(initial_value)
...
class DynamicTimeSmoothing(TimeSmoothing):
    ...
    def __init__(self, period, phase, window_factor=1.0, initial_value=0.0 + 0.0j, initial_delta=0.0):
...
class ApexTimeSmoothing(DynamicTimeSmoothing):
    ...
    def __init__(self, period, phase, window_factor=1.0, initial_value=0.0 + 0.0j, initial_delta=0.0, prior_value=None,
                 prior_sequence=None):
...
class PeriodPercept:
    ...
    def produce_monochord_percept(self, monochord):
        ...
        return PeriodPercept(monochord.target_period, self.period_factor, self.glissando_factor, self.time,
                             rotated_value, rotated_r, rotated_phi)
    ...
    def __str__(self):
        return "%010.3f + %010.3f / %010.3f: r=%08.3f[%s] phi=%08.3f[%s]" % (
            self.period, self.glissando_factor, self.period_factor, self.r, bar.bar(self.r, self.period), self.phi,
            bar.bar_log(self.phi + 0.5, 1.0))
...
class PeriodRecept:
    ...
    def __str__(self):
        return "%010.3f @ %010.3f + %010.3f / %08.3f: r=%08.3f[%s] r_d=%08.3f[%s] phi_t=%08.3f[%s]" % (
            self.instant_period, self.period, self.glissando_factor, self.period_factor, self.phase.r,
            bar.bar_log(self.phase.r, self.period), self.r_d, bar.signed_bar_log(self.r_d, 1.0 / self.period * 8),
            self.phi_t, bar.signed_bar(self.phi_t, 0.5))
...
class PeriodConcept:
    ...
    def __init__(self, sensor, weight_factor=1.0):
        ...
        self.avg_instant_period = 0.0
        self.avg_instant_period_offset = 0.0
        self.instant_period_delta = 0.0
        self.instant_period_stddev = 0.0
        self.recept = None
        self.percept = None
    ...
    def receive(self):
        ...
        self.avg_instant_period = self.avg_instant_period_state.sample(self.recept.instant_period,
                                                                       self.recept.period * self.weight_factor)
        ...
        self.instant_period_stddev = self.instant_period_stddev_state.sample(abs(self.instant_period_delta), abs(
            self.recept.instant_period * self.weight_factor))

        # self.avg_instant_distance =
        # self.avg_instant_distance_state.sample(self.recept.instant_distance, self.recept.period * self.weight_factor)
    ...
    def __str__(self):
        return "%010.3f / %010.5f <- %s" % (
            ...
            self.recept,
            # 1.0 / abs(self.avg_instant_distance)
            # if self.avg_instant_distance != 0
            # else 1.0,
            # bar.bar(1.0 / abs(self.avg_instant_distance)
            # if self.avg_instant_distance != 0 else 1.0, 1.0),
    ...
    def consonant(self, other, consonance_factor=1.0, harmonic=1):
        ...
        return ratio > (1.0 + 1.0 / self.weight_factor) * consonance_factor
...
class ApexPeriodSensor(PeriodSensor):
    def __init__(self, period, phase, period_factor=1.0, glissando_factor=0.0, phase_factor=1.0,
                 initial_value=0.0 + 0.0j):
...
class PeriodScaleSpaceSensor:
    def __init__(self, period, phase, response_period, scale_factor=1.75, period_factor=1.0, phase_factor=1.0,
                 initial_value=0.0 + 0.0j):
        self.period = period
        self.phase = phase
    ...
    def sample_lifecycle(self):
        self.period_lifecycle.sample(self.period_sensors[0].concept.percept.r, self.period_sensors[1].concept.percept.r,
                                     self.period_sensors[2].concept.percept.r)
...
class PeriodArray:
    ...
    def add_period_sensor(self, period, monochord_source_sensor=None, monochord_ratio=None, bandwidth_factor=1.0):
        period_sensor = PeriodScaleSpaceSensor(period, 0.0, self.response_period, self.scale_factor,
                                               self.period_bandwidth * bandwidth_factor, self.phase_factor)
...
class LogPeriodArray(PeriodArray):
    def __init__(self, period, response_period, octaves=1, octave_bandwidth=12, scale_factor=1.75, period_factor=1.0,
                 phase_factor=1.0):
        self.period = period
...
class UkePeriodArray(PeriodArray):
    def __init__(self, sample_rate, response_period, octave_bandwidth=36, scale_factor=1.75, period_factor=1.0,
                 phase_factor=1.0):
...
class LinearPeriodArray(PeriodArray):
    def __init__(self, sampling_rate, response_period, scale_factor=1.75, start_frequency=50, stop_frequency=2000,
                 step_frequency=50, period_factor=1, phase_factor=1):
        self.period_factor = period_factor
        self.phase_factor = phase_factor
        self.period_sensors = [
            PeriodScaleSpaceSensor(1.0 / (float(frequency) / sampling_rate), 0.0, response_period, scale_factor,
                                   period_factor / (float(step_frequency) / sampling_rate) * (
                                           float(frequency) / sampling_rate), phase_factor)
...
class DurationScaleSpaceSensor:
    def __init__(self, target_duration, window_size, response_period, scale_factor=1.75, prior_value=None,
                 initial_duration=0.0, initial_value=0.0, prior_sequence=0.0):
        self.target_duration = target_duration
        self.window_size = window_size
        self.response_period = response_period

        self.duration_sensors = [
            SmoothDurationDistribution(target_duration * scale_factor ** (-scale), window_size, prior_value,
                                       initial_duration, initial_value, prior_sequence)
            for scale in range(3)
        ]
    ...
    def sample_lifecycle(self):
        ...
        self.period_lifecycle.sample(reciprocate(self.duration_sensors[0].v.dev.v),
                                     reciprocate(self.duration_sensors[1].v.dev.v),
                                     reciprocate(self.duration_sensors[2].v.dev.v))
        self.beat_lifecycle.sample(self.period_lifecycle.lifecycle)
...
class LogDurationArray(DurationArray):
    def __init__(self, target_duration, window_size, response_period, scale_factor=1.75, scale=12, octaves=1):
        self.response_period = response_period
        self.scale_factor = scale_factor
        self.duration_sensors = [
            DurationScaleSpaceSensor(target_duration * 2 ** (float(n) / scale), octaves, window_size, response_period,
                                     scale_factor)
...
def note(sample_rate, period, A4=440.0):
    notes = ["C /B#", "C#/Db", "D /D ", "D#/Eb", "E /Fb", "F /E#", "F#/Gb", "G /G ", "G#/Ab", "A /A ", "A#/Bb", "B /Cb"]
    from math import floor
    n = midi_note(sample_rate, period, A4)

    int_n = int(floor(n + 0.5))

    octave = int_n / 12 - 1
    octave_note = int_n % 12
...
def periodic_test(generate=False):
    ...
    while True:
        ...
        if generate:
            ...
            # read from file
            def get_sample(fs2):
                return float(fs2.next()) / (oversample * (2 ** 32)) * 10000
        ...
        for pos, pa in enumerate(pa_octaves):
            ...
            for period_sensor in pa.period_sensors:
                ...
                if draw:
                    pc = (-lc.dd_avg if lc.dd_avg < 0 else 0) + (lc.F * 1j if lc.F < 0 else 0)
                    power = abs(pc)
                    sampler.screen.printf(
                        "{:9} {:9} {:14} {} \n",
